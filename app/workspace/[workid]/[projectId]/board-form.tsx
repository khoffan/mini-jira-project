...
export default function BoardForm({ workspaceId, projectId, onClose }: BoardFormProps) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setLoading(true)
        try {
            await createBoardAction({ title, projectId, workspaceId })
            router.refresh()
            onClose()
        } catch (error) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                {/* Header */}
                <div className="px-6 py-5 border-b border-slate-100 bg-linear-to-r from-emerald-50 to-teal-50">
                    <h2 className="text-xl font-bold text-slate-900">
                        สร้างบอร์ดใหม่
                    </h2>
                    <p className="text-sm text-slate-500 mt-1">
                        กรอกข้อมูลเพื่อสร้างบอร์ดใหม่ในโปรเจกต์
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    <div className="space-y-1.5">
                        ...
                        <input
                            ...
                            placeholder="เช่น Sprint Backlog, Feature Requests"
