...
import { INestedBoard, IProject, ITask, IWorkspaceWithNestedData } from "@/lib/types";
...
export default async function BoardPage({ params }: PageProps) {
  ...
  const workspace = await Workspace.findOne({ slug }).lean<IWorkspaceWithNestedData>();
  ...
  const board = await Board.findOne({ slug: boradSlug })
    .populate({
      path: "tasks",
      model: Task,
    })
  ...
  const taskStats = {
    total: board.tasks ? board.tasks.length : 0,
    todo: board.tasks ? board.tasks.filter((t: ITask) => t.status === "TODO").length : 0,
    inProgress: board.tasks ? board.tasks.filter((t: ITask) => t.status === "IN_PROGRESS").length : 0,
    done: board.tasks ? board.tasks.filter((t: ITask) => t.status === "DONE").length : 0,
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="app-container py-6 sm:py-8 lg:py-10">
        ...
        <Link
          href={`/workspace/${slug}/${projectSlug}/${boradSlug}/task`}
