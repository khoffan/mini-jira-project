...
export default function WorkspaceSubNav({ workspace, allWorkspaces = [] }: WorkspaceSubNavProps) {
  const router = useRouter();
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);
  const { view, setView } = useViewStore();
  const project = useProjectStore((s) => s.project);
  const boards = useBoardStore((s) => s.board);
  ...
  const handleInviteClick = async () => {
    ...
    if (!code) {
      toast.error("ไม่มีรหัสเชิญสำหรับ workspace นี้");
      return;
    }
    try {
      ...
      // fallback: show prompt to allow manual copy
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1 overflow-visible">
          ...
          <div className="relative shrink-0" ref={dropdownRef}>
            ...
            >
              ...
              <span className="font-medium text-slate-700 text-xs sm:text-sm truncate max-w-20 sm:max-w-36">
                {workspace.name}
              </span>
              <ChevronDown
                size={14}
              ...
              <div className="absolute top-full left-0 mt-1.5 w-64 bg-white rounded-xl shadow-xl border border-slate-200/80 py-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                ...
                <div className="max-h-52 overflow-y-auto py-1">
                  {allWorkspaces.map((ws) => (
                    ...
                    >
                      <div
                        className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold ${ws.id === workspace.id ? "bg-indigo-100 text-indigo-600" : "bg-slate-100 text-slate-500"}`}
                      >
                        {ws.name.charAt(0).toUpperCase()}
                      </div>
                      <span
                        className={`text-sm truncate flex-1 ${ws.id === workspace.id ? "font-semibold text-indigo-700" : "text-slate-700"}`}
                      >
                        {ws.name}
                      </span>
                      {ws.id === workspace.id && (
          ...
          {project && boards && (
            <>
              <ChevronRight size={14} className="text-slate-300 shrink-0" />
              <Link
                href={`/workspace/${workspace.slug}/${project.slug}/${boards.slug}`}
                className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 hover:bg-slate-100/80 rounded-lg cursor-pointer border border-slate-200/80 transition-colors shrink-0"
              >
                <div className="w-5 h-5 sm:w-6 sm:h-6 bg-emerald-100 text-emerald-600 rounded flex items-center justify-center text-[10px] sm:text-xs font-bold">
                  {boards.title.charAt(0).toUpperCase()}
                </div>
                <span className="font-medium text-slate-700 text-xs sm:text-sm truncate max-w-20 sm:max-w-36">
                  {boards.title}
                </span>
          ...
          <div className="hidden md:flex items-center gap-2 ml-2">
            ...
            <nav className="flex items-center gap-1 ml-1">
              <button className="px-3 py-1.5 text-xs sm:text-sm font-medium text-indigo-600 bg-indigo-50 rounded-md">
                Boards
              </button>
              <button className="px-3 py-1.5 text-xs sm:text-sm font-medium text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded-md transition-all">
                Members
              </button>
            </nav>
          </div>
